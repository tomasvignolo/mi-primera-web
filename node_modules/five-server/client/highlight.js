...
function _screenOffset(element) {
    const elemBounds = element.getBoundingClientRect(), body = window.document.body;
...
export class Highlight {
    ...
    makeHighlightDiv(element, doAnimation) {
        const elementBounds = element.getBoundingClientRect(), highlight = window.document.createElement('div'), elementStyling = window.getComputedStyle(element), transitionDuration = parseFloat(elementStyling.getPropertyValue('transition-duration')), animationDuration = parseFloat(elementStyling.getPropertyValue('animation-duration'));
        ...
        let innerWidth = parseFloat(elementStyling.width), innerHeight = parseFloat(elementStyling.height), outerHeight = innerHeight, outerWidth = innerWidth;
        if (!borderBox) {
        ...
        const setupVisualizations = function (arr, config) {
            ...
            for (i = 0; i < arr.length; i++) {
                ...
                const el = window.document.createElement('div'), styles = Object.assign({}, config, arr[i]);
        ...
        let el = element, offsetLeft = 0, offsetTop = 0;
    ...
    clear() {
        let i;
        const highlights = window.document.querySelectorAll('.' + HIGHLIGHT_CLASS_NAME), body = window.document.body;
